...
#ifndef SIMULATION_WS_FIND_DESIRABLE_LOCATION_H
#define SIMULATION_WS_FIND_DESIRABLE_LOCATION_H
...
//--------------------------------------------------------------------------------
#define OBJECTNESS_THRESHOLD 0.5          //tables above this threshold are only considered for plane fitting
#define OVERLAPPING_POINTS 10               // number of points required to considered an overlap of two BB
...
#define STATISTICAL_OUTLIER_STD 2.0            // Standard Deviation multiplier is 2 - all points who have distance larger than 2 std of the mean distance to the query point is marked as outlier
...
#define REGION_GROWING_OVERLAPPING 50
...
public:
    //constructor
    DesirableLocations(pcl::PointCloud<pcl::PointXYZ>::Ptr scene_cloud, cv::Mat &d_image,
                      std::vector <desirable_locations::votenetDetection> detections);
    //destructor
    ~DesirableLocations();

    //connect two edges in the polygon
    std::vector<Eigen::Vector3f> addInbetweenPaddedLines(std::vector<Eigen::Vector3f> padded_approx_polygon);

    //fit toilet model
    void fitModelToilets(pcl::PointCloud<pcl::PointXYZ>::Ptr toilet_pcd,
                                        pcl::ModelCoefficients::Ptr coefficients);

    //helper functions for merging overlapping bounding boxes.
    ...
    //extract the point cloud based on label names
    std::map<int, pcl::PointCloud<pcl::PointXYZ>::Ptr> extract_pointcloud(std::string label_name, std::vector <desirable_locations::votenetDetection> &detections);
    std::map<int, pcl::PointCloud<pcl::PointXYZ>::Ptr> extract_toilet_bbox(std::string label_name, std::vector <desirable_locations::votenetDetection> &detections);

    //merge the extracted point cloud
    void findIntersectingBBoxTables(std::map<int, pcl::PointCloud<pcl::PointXYZ>::Ptr> &extracted_pcd, std::map<std::string, pcl::PointCloud<pcl::PointXYZ>::Ptr> &merged_bbox, std::string label_name);
    void findIntersectingBBoxToilets(std::map<int, pcl::PointCloud<pcl::PointXYZ>::Ptr> &extracted_bbox, std::map<std::string, pcl::PointCloud<pcl::PointXYZ>::Ptr> &merged_bbox, std::string label_name);

    std::vector <Eigen::Vector3f> findToiletPolygon(pcl::PointCloud<pcl::PointXYZ>::Ptr box_corners);
    ...
    std::vector <std::vector<double>> calculateVisibilityWeights(std::vector <std::vector<Eigen::Vector3f>> locations, std::vector<double> wheelchair_dimensions);
...
#endif //SIMULATION_WS_FIND_DESIRABLE_LOCATION_H
